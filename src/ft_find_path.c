...
/*   Updated: 2020/07/24 12:29:20 by val              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_path *ft_find_path(t_room *head_culc, t_storage *st)
{
	t_path	*result;
	int		count_rooms;
	int		counter;
...
void	ft_breadth_first_search_uncrosing(t_room *head, int key, int power, int key_start)
{
	...
	{
		key_in = (head[key]).tube_in[counter];
		if (key_in != INVALID_TUBE && (head[key]).weight_in[counter] != PRIORITY_WEIGTH)
		{
			if (key_in == key_start)
				return;
			if ((head[key_in]).vertex == -1 || (head[counter]).vertex > power)
...
t_path *ft_find_path_uncrosing(t_room *head_culc, t_storage *st)
{
	t_path	*result;
	int		count_rooms;
	int		counter;
